---
name: repolens
description: Use the RepoLens model-free external proposal workflow for grouped shortlist resolution.
---

Read and follow `../../../.skills/repolens/SKILL.md`. Treat that file as the source of
truth. RepoLens itself must remain model-free; this wrapper only enables Claude to create
external proposal artifacts that RepoLens later verifies and a human approves.
